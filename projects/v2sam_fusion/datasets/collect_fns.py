...
def video_lisa_collate_fn(instances: Sequence[Dict],
    ...
    has_prompt_image = any(inst.get('prompt_pixel_values') is not None for inst in instances)
    ...
    has_prompt_grounding_image = any(inst.get('prompt_g_pixel_values') is not None for inst in instances)
    ...
    has_raw_prompt_mask = any(inst.get('raw_prompt_masks') is not None for inst in instances)
    
    # wrz: 添加路径字段检测（用于sparse correspondence）
    has_query_img_path = any(inst.get('query_img_path') is not None for inst in instances)
    has_target_img_path = any(inst.get('target_img_path') is not None for inst in instances)
    has_query_mask_path = any(inst.get('query_mask_path') is not None for inst in instances)
    ...
    if has_prompt_image:
        prompt_pixel_values = []
        prompt_frames_per_batch = []
        prompt_image_grid_thw = []
    if has_grounding_image:
        grounding_pixel_values = []
    if has_prompt_grounding_image:
        prompt_grounding_pixel_values = []
    if has_mask:
        object_masks = []
    if has_bboxes:
        object_bboxes = []
    ...
    if has_raw_prompt_mask:
        raw_prompt_masks = []
    else:
        raw_prompt_masks = None
    
    # wrz: 初始化路径列表
    if has_query_img_path:
        query_img_paths = []
    else:
        query_img_paths = None
    
    if has_target_img_path:
        target_img_paths = []
    else:
        target_img_paths = None
    
    if has_query_mask_path:
        query_mask_paths = []
    else:
    ...
    for example in instances:
        ...
        if has_prompt_image:
            prompt_pixel_values.append(example['prompt_pixel_values'])
            if has_pe:
                prompt_image_grid_thw.append(example['prompt_image_grid_thw'])
            if has_vp:
                if 'vp_overall_mask' in example.keys() and example['vp_overall_mask'] is not None:
                    vp_overall_mask.append(example['vp_overall_mask'])
                else:
                    vp_overall_mask.append(torch.Tensor([False] * len(prompt_pixel_values[-1])))
        if has_fast_image:
        ...
        if has_prompt_grounding_image and 'prompt_g_pixel_values' in example.keys():
            if isinstance(example['prompt_g_pixel_values'], list):
                prompt_grounding_pixel_values += example['prompt_g_pixel_values']
                prompt_frames_per_batch.append(len(example['prompt_g_pixel_values']))
            else:
                prompt_grounding_pixel_values.append(example['prompt_g_pixel_values'])
                prompt_frames_per_batch.append(1)
        if has_mask:
        ...
        if has_raw_prompt_mask:
            if 'raw_prompt_masks' in example.keys():
                raw_prompt_masks.append(example['raw_prompt_masks'])
        
        # wrz: 收集路径字段
        if has_query_img_path:
            if 'query_img_path' in example.keys():
                query_img_paths.append(example['query_img_path'])
        
        if has_target_img_path:
            if 'target_img_path' in example.keys():
                target_img_paths.append(example['target_img_path'])
        
        if has_query_mask_path:
            if 'query_mask_path' in example.keys():
                query_mask_paths.append(example['query_mask_path'])
    ...
    if has_prompt_image:
        if all(x.shape == prompt_pixel_values[0].shape for x in prompt_pixel_values):
            prompt_pixel_values = torch.stack(prompt_pixel_values, dim=0)
        data_dict['prompt_frames_per_batch'] = prompt_frames_per_batch
        data_dict['prompt_pixel_values'] = prompt_pixel_values
        if has_pe:
            data_dict['prompt_image_grid_thw'] = prompt_image_grid_thw
    ...
    if has_raw_prompt_mask:
        data_dict['raw_prompt_masks'] = raw_prompt_masks
    ...
    if has_prompt_grounding_image:
        # if all(x.shape == grounding_pixel_values[0].shape for x in grounding_pixel_values):
            # grounding_pixel_values = torch.stack(grounding_pixel_values, dim=0)
        data_dict['prompt_g_pixel_values'] = prompt_grounding_pixel_values
    ...
    # wrz: 添加路径字段到data_dict（保持列表格式，在model中提取第一个元素）
    if has_query_img_path and query_img_paths:
        data_dict['query_img_path'] = query_img_paths
    
    if has_target_img_path and target_img_paths:
        data_dict['target_img_path'] = target_img_paths
    
    if has_query_mask_path and query_mask_paths:
        data_dict['query_mask_path'] = query_mask_paths

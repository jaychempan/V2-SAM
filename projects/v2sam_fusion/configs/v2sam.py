...
#                          PART 1  Settings                           #
#######################################################################
# pretrained_pth = "path/epoch_32.pth" # debug
# sam2_original_ckpt = "/path/V2-SAM/third_parts/sam2/sam2_configs/sam2_hiera_large.pt"  # debug: 原始SAM2权重（用于sparse points预测）
pretrained_pth = None
...
model = dict(
    ...
    grounding_encoder=dict(
        ...
        base_dir='weights/sam2',  # relative to repo root; set absolute if preferred
    ),
    loss_mask=dict(
    ...
    dinov3_cfg=dict(
        ...
        repo_path='third_parts/dinov3',
        # Provide a path relative to repo root or an absolute path on your system
        weights_path='weights/dinov3/dinov3_vitl16_pretrain_lvd1689m-8aa4cbdd.pth',
    ),
    sparse_corr_cfg=dict(
        patch_size=16,
        image_size=768,
        mask_fg_threshold=0.6,
        stratify_distance_threshold=150.0,
        n_layers=24,
        max_points_per_object=1,
        outlier_removal_ratio=0.25,
    ),
)
...
#     sam2_folder='/work/jiancheng_pan/datasets/EgoOmni-Pretraining/SA-V/sav_train',
...
#     # sam2_folder='/path/EGP/Ego-Exo4D-Seg/data_segswap',
#     # expression_file='/path/EGP/json/Ego-Exo4D-Seg/frame-level/Ego2Exo_FullTrain.json',
#     sam2_folder='/path/EGP/Ego-Exo4D-Seg/data_segswap',
#     expression_file='/path/EGP/json/Ego-Exo4D-Seg/frame-level/Exo2Ego_FullTrain.json',
#     template_map_fn=dict(
#         type=template_map_fn_factory, template=prompt_template),
#     max_length=max_length,
#     lazy=True,
...
#     type=VideoObjectRelatorDataset,
#     # sam2_folder='/path/EGP/Ego-Exo4D-Seg/data_segswap',
#     # expression_file='/path/EGP/json/Ego-Exo4D-Seg/frame-level/ego2exo_val_framelevel.json',  # <-- 验证集json
#     # sam2_folder='/path/EGP/Ego-Exo4D-Seg/data_segswap_test',
#     # expression_file='/path/EGP/json/Ego-Exo4D-Seg/frame-level/ego2exo_test_framelevel.json',  # <-- 测试集json
#     sam2_folder='/path/EGP/Ego-Exo4D-Seg/data_segswap_test',
#     expression_file='/path/EGP/json/Ego-Exo4D-Seg/frame-level/exo2ego_test_framelevel.json',  # <-- 测试集json
#     template_map_fn=dict(
#         type=template_map_fn_factory, template=prompt_template),
#     max_length=max_length,
#     lazy=True,
